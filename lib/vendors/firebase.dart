...
extension AppFirebaseAuth on FirebaseAuth {
  Future<void> verificationCompletedCallback({
    ...
    try {
      if (firebaseAuth.currentUser != null) {
        // if user is logged in, it means we want to update the phone number
  ...
  Future<DeleteAccountResponse> deleteAccount({
    ...
    try {
      ...
      firebase.database.deleteUser(this.currentUser.uid);

      // delete user data from firebase Storage
      firebase.storage.deleteUser(this.currentUser.uid);
    ...
    reasons.keys.forEach((key) async {
      ...
      switch (key) {
        ...
        case DeleteReason.badRideExperience:
          reasonString = "bad-ride-experience";
          break;
...
extension AppFirebaseStorage on FirebaseStorage {
  ...
  Future<ProfileImage> getProfileImage({@required String uid}) async {
    ListResult results =
        await this.ref().child("user-photos").child(uid).list();
